...
HBITMAP GetImageBitmapForPath(const std::string &name, uint32_t width,
                              uint32_t height, uint32_t flags) {
  HBITMAP hbmp = NULL;
  PCWSTR errorMessage = NULL;

  IShellItemImageFactory *pImageFactory;
  HRESULT hr = SHCreateItemFromParsingName(Utf8ToWide(name).c_str(), NULL,
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (FAILED(hr)) {
      errorMessage =
          L"IShellItemImageFactory::GetImage failed with error code %x";
    }
...
CreateBitmapFromHBitmap(HBITMAP hBitmap, std::vector<std::int32_t> &buffer) {
  BITMAP bm = {0};
...
std::vector<unsigned char> GetImageForPath(const std::string &name,
